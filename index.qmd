...
-   **Model Selection:** Rigorously compare standard linear models against the Zero-Inflated Gamma framework using AIC, BIC, and DHARMa residual diagnostics. \# **Computational Environment and Data Ingestion**
...
m0_null <- readRDS("models/m0_null.rds") # Baseline
m1_moisture <- readRDS("models/m1_moisture.rds") # + Humidity/Pressure
m2_temporal <- readRDS("models/m2_temporal.rds") # + Seasonality/Markov
m3_history <- readRDS("models/m3_history.rds") # + Moving Averages
m4_energy <- readRDS("models/m4_energy.rds") # + Sun/Evap/Interactions
m5_wind <- readRDS("models/m5_wind.rds") # + Wind Vectors

m6_mixed <- readRDS("models/m6_mixed.rds") # + Random Effects (Best Model)

m_linear <- readRDS("models/m_linear.rds") # Gaussian (Fails)
m_tweedie <- readRDS("models/m_tweedie.rds") # Tweedie (Good, but not best)

check_data <- readRDS("data/prediction_check_data.rds") # For Distribution plots
ppc_data <- readRDS("data/ppc_data.rds") # For Posterior Checks
```
...
lrt_results <- tibble(
  ...
  mutate(
    Significant = ifelse(p_value < 0.001, "***", "ns"),
    AIC_improvement = c(
...
#| tbl-cap: "Comprehensive Model Selection Table"
#| echo: true
...
model_selection %>%
  ...
  kable(caption = "Comprehensive Model Selection Table") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
